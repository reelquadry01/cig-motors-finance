...
function LeafRow({ label, value, revenueTotal, indent = 44 }) {
  return (
    <tr style={{ animation: 'fadeIn 0.15s ease-out' }}>
      ...
      <td style={{ textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{fmt.th(value)}</td>
  ...
  return (
    <>
      <tr className="collapse-trigger" onClick={() => setOpen(!open)}>
        ...
        <td style={{ textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontWeight: 500 }}>{fmt.th(group.value)}</td>
        <td style={{ textAlign: 'right' }}>—</td>
        <td style={{ textAlign: 'right' }}>—</td>
        <td style={{ textAlign: 'right' }}>
          <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
  ...
  if (!expandable) {
    return (
      <tr className={bold ? 'row-subtotal' : ''}>
        ...
        <td style={{ textAlign: 'right', fontWeight: bold ? 700 : 400, fontVariantNumeric: 'tabular-nums' }}>{fmt.th(total)}</td>
  ...
  return (
    <>
      <tr className="collapse-trigger" onClick={() => setOpen(!open)}>
        ...
        <td style={{ textAlign: 'right', fontWeight: bold ? 700 : 500, fontVariantNumeric: 'tabular-nums' }}>{fmt.th(total)}</td>
        <td style={{ textAlign: 'right' }}>{margin !== undefined && <span className="badge badge-neutral">{fmt.pct(margin)}</span>}</td>
        <td style={{ textAlign: 'right' }}>—</td>
