...
export default function ConnectorStep1({
  ...
  const handleConnect = async () => {
    setConnecting(true)
    try {
      await onConnect(localConfig, localCreds, localName)
    } finally {
      setConnecting(false)
    }
  }
  ...
  if (loading) {
    return (
      <div className="grid place-items-center py-16 text-neutral-500">
        ...
        <span className="text-sm">Loading connection fields…</span>
      </div>
    )
  }
  ...
  if (!schema || schema.length === 0) {
    return (
      <div className="text-center py-16 text-sm text-neutral-500">
        ...
        No configuration fields available for this connector type.
      </div>
    )
  }
      ...
      <div>
        <label className="block">
          ...
          <input
            ...
            placeholder={subtype ? `${subtype} connection` : `${typeKey} connection`}
            className="mt-1.5 w-full rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-800 px-3 py-2.5 text-sm text-neutral-900 dark:text-neutral-100 focus:border-[#1f3a5f] focus:ring-2 focus:ring-[#1f3a5f]/20 focus:outline-none transition-colors"
          />
        </label>
      </div>
      ...
      <label className="block">
        ...
        {field.type === 'select' ? (
          <select
            ...
            className="mt-1.5 w-full rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-800 px-3 py-2 text-sm text-neutral-900 dark:text-neutral-100 focus:border-[#1f3a5f] focus:ring-2 focus:ring-[#1f3a5f]/20 focus:outline-none transition-colors"
          >
          ...
          <textarea
            ...
            rows={4}
            className="mt-1.5 w-full rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-800 px-3 py-2 text-sm text-neutral-900 dark:text-neutral-100 font-mono focus:border-[#1f3a5f] focus:ring-2 focus:ring-[#1f3a5f]/20 focus:outline-none transition-colors resize-y"
          />
        ) : (
          <div className="relative">
            <input
              ...
              onChange={(e) => onChange(e.target.value)}
              placeholder={field.placeholder}
              className={`mt-1.5 w-full rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-800 px-3 py-2 text-sm text-neutral-900 dark:text-neutral-100 focus:border-[#1f3a5f] focus:ring-2 focus:ring-[#1f3a5f]/20 focus:outline-none transition-colors ${isPassword ? 'pr-10' : ''}`}
            />
            {isPassword && (
              <button
                type="button"
